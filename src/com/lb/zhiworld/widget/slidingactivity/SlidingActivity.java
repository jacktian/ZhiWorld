package com.lb.zhiworld.widget.slidingactivity;
...
public class SlidingActivity extends FragmentActivity {
	...
	public void setContentView(int layoutResID) {
		...
		final int marginTop = hideTitle ? 0
				: (int) (metrics.density * 48.f + .5f);
		...
		slideLayout
				.setPanelSlideListener(new SlidingLayout.SimpleSlideListener() {
					@Override
		...
		byte[] byteArray = getIntent().getByteArrayExtra(
				IntentUtils.KEY_PREVIEW_IMAGE);
		if (null != byteArray && byteArray.length > 0) {
			try {
				bmp = BitmapFactory.decodeByteArray(byteArray, 0,
						byteArray.length);
				if (null != bmp) {
